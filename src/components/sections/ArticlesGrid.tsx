...
function BlogArticleCard({ article }: { article: BlogArticle }) {
  ...
  return (
    <GlassCard radius="2xl" className="group flex flex-col overflow-hidden">
      <Link href={href} className="flex flex-1 flex-col" aria-label={article.title}>
        <div className="relative aspect-[4/3] overflow-hidden">
          ...
          <span className="absolute left-4 top-4 rounded-full border border-primary/20 bg-background/70 px-3 py-1 text-small text-primary backdrop-blur-md rtl:left-auto rtl:right-4">
            {article.category}
          </span>
        </div>
        <div className="flex flex-1 flex-col p-6">
          ...
          <span className="mt-4 flex items-center gap-2 text-small text-primary">
