    ...
    <section className="mx-auto mb-20 max-w-container-max px-margin-mobile md:px-margin-desktop">
      ...
      >
        ...
        <div className="relative z-10 w-full space-y-6 md:w-2/3">
          <Reveal>
            <h2 className="text-[42px] font-bold leading-tight text-white">{heading}</h2>
          </Reveal>
          <Reveal delay={0.08}>
            <p className="text-body-lg text-on-surface-variant">{subtitle}</p>
          </Reveal>
          <Reveal delay={0.16} className="flex flex-wrap justify-center gap-4 md:justify-start">
            <Button size="lg" className="px-12 py-5 shadow-2xl">
              {primaryLabel}
            </Button>
            {showWhatsapp && (
              <Button
