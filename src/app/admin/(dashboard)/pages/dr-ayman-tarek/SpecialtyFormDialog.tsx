...
export function SpecialtyFormDialog({ item, nextOrder, onClose }: { item: SpecialtyRow | null | "new"; nextOrder: number; onClose: () => void }) {
  const [form, setForm] = useState(emptyForm);
  const [slugTouched, setSlugTouched] = useState(false);
  const [image, setImage] = useState<MediaAsset | null>(null);
  const create = specialtyHooks.useCreate();
  const update = specialtyHooks.useUpdate();
    ...
    <Dialog open={open} onClose={onClose} className="max-w-2xl">
      <div className="max-h-[85vh] overflow-y-auto p-6">
        <div className="mb-5 flex items-center justify-between">
          <h2 className="text-lg font-bold text-white">{item === "new" ? "Add Specialty" : "Edit Specialty"}</h2>
          {item !== "new" && <VersionHistoryButton table="specialties" entityType="specialties" entityId={item?.id} />}
        </div>
        <div className="space-y-5">
          ...
          <MediaPickerField
            ...
            onChange={(asset) => { setImage(asset); setForm((f) => ({ ...f, image_media_id: asset?.id ?? null })); }}
          />
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <BilingualField
              ...
              placeholder={{ en: "e.g. 1.5 – 3 hours" }}
